...
const frontendUrl =
  ...
  'https://habeshaimexai-cyber.github.io/creatoros-ai/';
...
const has = (keys) =>
  [...coreRequired, ...keys].every((key) => Boolean(process.env[key]));
...
function readState(state) {
  ...
  if (!payload.userId || payload.exp < Date.now()) {
    throw new Error('Status abgelaufen');
  }
...
function finish(res, message, success = false) {
  res.type('html').send(
    '<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1">' +
      ...
      '<h1>' +
      (success ? 'YouTube verbunden' : 'Verbindung fehlgeschlagen') +
      '</h1><p>' +
      message +
      '</p><p>Du kannst dieses Fenster schließen und zu CreatorOS zurückgehen.</p></main>'
  );
}
...
async function saveConnection(connection) {
  const response = await fetch(
    process.env.SUPABASE_URL +
      '/rest/v1/social_connections?on_conflict=user_id,platform,account_id',
    {
      method: 'POST',
      headers: {
        apikey: process.env.SUPABASE_SERVICE_ROLE_KEY,
...
app.get('/connections', async (req, res) => {
  ...
  const response = await fetch(
    process.env.SUPABASE_URL +
      ...
      '?select=platform,account_id,handle,status' +
      '&user_id=eq.' +
...
app.post('/oauth/youtube/start', async (req, res) => {
  if (!has(youtubeRequired)) {
    return res.status(503).json({
      error: 'Die sichere YouTube-Verbindung wird noch eingerichtet.',
    });
  }
  ...
  if (!creator?.id) {
    return res.status(401).json({
      error: 'Bitte zuerst bei CreatorOS anmelden.',
    });
  }
...
app.get('/oauth/youtube/callback', async (req, res) => {
  try {
    if (!has(youtubeRequired)) {
      throw new Error(
        'Der Server ist noch nicht vollständig eingerichtet.'
      );
    }

    if (!req.query.code || !req.query.state) {
      throw new Error(
        'Google hat keine Berechtigung zurückgegeben.'
      );
    }
    ...
    if (!tokenResponse.ok || !tokens.access_token) {
      throw new Error(
        'Google-Berechtigung wurde nicht akzeptiert.'
      );
    }
    ...
    if (!channel?.id) {
      throw new Error(
        'Kein YouTube-Kanal für dieses Google-Konto gefunden.'
      );
    }
    ...
    finish(
      res,
      'Dein YouTube-Kanal wurde sicher mit deinem CreatorOS-Konto verbunden.',
      true
    );
  } catch (error) {
    finish(
...
app.post('/oauth/facebook/start', async (req, res) => {
  if (!has(metaRequired)) {
    return res.status(503).json({
      error: 'Die sichere Facebook-Verbindung wird noch eingerichtet.',
    });
  }
...
app.get('/oauth/facebook/callback', async (req, res) => {
  try {
    if (!has(metaRequired)) {
      throw new Error(
        'Der Facebook-Server ist noch nicht eingerichtet.'
      );
    }

    if (!req.query.code || !req.query.state) {
      throw new Error(
        'Facebook hat keine Berechtigung zurückgegeben.'
      );
    }
    ...
    if (!tokenResponse.ok || !tokens.access_token) {
      throw new Error(
        'Facebook-Berechtigung wurde nicht akzeptiert.'
      );
    }
    ...
    if (!profileResponse.ok || !profile.id) {
      throw new Error(
        'Facebook-Profil konnte nicht gelesen werden.'
      );
    }
    ...
    finish(
      res,
      'Dein Facebook-Konto wurde sicher mit CreatorOS verbunden.',
      true
    );
  } catch (error) {
    finish(
...
app.listen(process.env.PORT || 3001, () => {
  console.log('CreatorOS API läuft');
});
